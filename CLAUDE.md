...
- `state_store.py` — `StateStore` wraps SQLite at `.local/state/cockpit.db` (schema versioned, currently v11). Migrations run on `__init__`; `_migrate_vN_to_vN+1` methods are append-only — never edit a past migration. Key entities:
  - **aesthetic_jobs** — named optimization targets, each with a `seed_system_prompt`, `seed_candidate_id`, `latest_system_prompt`, `baseline_system_prompt`, `active_candidate_id`, and a mutation threshold. Creating or migrating a job inserts the seed prompt into `gepa_candidates` as an evaluated seed candidate (always in the contender pool).
  ...
  - **gepa_candidates** — seed prompts and system prompt versions produced by prompt-mutation runs. Candidates have statuses `proposed`, `evaluating`, `evaluated`, or `archived`; track `elo`, `score`, `confidence`, win/loss/tie counts, judge metadata, parent IDs, and `frontier_member` (contender-pool membership, recomputed after feedback).
...
- `reward.py` — pure Elo updates, normalized preference score (`preference_score_from_elo`), and confidence-from-evidence helpers for human head-to-head candidate evaluation.
...
- `optimizer.py` — `run_gepa_optimization` is now the prompt-mutation worker kept under the existing `gepa` run type for compatibility. It loads selected feedback, picks a parent from evaluated contenders (`frontier_member`) when available, creates one proposed candidate, and writes a checkpoint. Promotion requires later human evaluation.
  ...
  - Training matchup selection prioritizes pending proposed/evaluating candidates against contender/evaluated candidates. If no pending candidates exist, evaluated non-seed candidates are compared against the seed candidate before falling back to active/latest-vs-baseline.
  ...
  - Candidate rewards are stored as `elo`, `score`, and `confidence`. `score` is the Elo-derived preference rank on [0,1] (`preference_score_from_elo`); `confidence` is evidence from evaluation count and average critique confidence. `frontier_member` marks the contender pool (seeds plus non-seeds with enough evidence and rank near the job leader). Promotion still requires an evaluated contender.
